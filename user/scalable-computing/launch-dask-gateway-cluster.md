...
- *How do I install software packages to my Dask hub environment?*
   ...
   During production, for best performance we advise adding the required software packages to the [Pangeo Docker image](https://pangeo-docker-images.readthedocs.io/en/latest/), ensuring that the key package `dask-gateway` is included (and `dask-labextension` if you require the Dask dashboard) – see [](/admin/environment/update-community-image.md) for guidance.
   Deploy your custom image to the hub by [contacting 2i2c support](../../support.md) or use [](#environment:bring-your-own-image) in the image selection dropdown if available.
