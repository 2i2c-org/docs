...
def create_summary_card(app, pagename, templatename, context, doctree):
    ...
    N_WORDS_DESCRIPTION = 30
    tagline = " ".join(text.split()[:N_WORDS_DESCRIPTION]) + "..."
    ...
    with plt.rc_context({"font.sans-serif": [font], "text.color": text_color}):
        ...
        txt_description = axtext.text(
            left_margin, 0.35, tagline, {"size": 13}, ha="left", va="top", wrap=True
        )
        txt_description._get_wrap_line_width = lambda: 500
        axtext.set_axis_off()

    axim = fig.add_axes((0.8, 0.8, 0.15, 0.15))
    axim.imshow(img)
    axim.set_axis_off()
