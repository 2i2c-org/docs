...
4. Select {gui}`From Scratch` in the popup that shows up.
5. Enter "Grafana Community Alerts (2i2c Hub)" (or a similarly descriptive name) under {gui}`App Name`. This will be the name of the bot that posts the alerts.
...
2. In the left panel, select {gui}`Alerting > Contact points`
...
3. Click {gui}`Create contact point`.
4. Select {gui}`Slack` under the {gui}`Integration` dropdown.
5. Enter a descriptive name for this contact point under {gui}`Name` (such as `2i2c-hub-alerts`).
6. Paste the webhook URL you copied in the previous section under {gui}`Webhook URL`.
7. Press the {gui}`Test` button and validate that you are able to see a message in the appropriate slack channel.
8. Click {gui}`Save Contact Point`.

### 3. Create a new dashboard folder

The default JupyterHub dashboards, available under the {gui}`JupyterHub Default Dashboards` folder
...
1. Select {gui}`Dashboards` on the left sidebar
2. Click {gui}`New` on the top right
3. Select {gui}`New folder` in the dropdown
4. Name the folder "Community Maintained Dashboards" and click the {gui}`Create` button.
...
1. Select {gui}`Alerting > Alert Rules` in the left sidebar.
2. Click the {gui}`New alert rule` button on the top right
3. Enter a descriptive name for this rule, like "Users approaching home directory limit"
4. Under {gui}`Define query and alert condition`, you'll have a space to
   enter a PromQl query. If you don't see such a space, you may have to toggle the
   switch on the right from {gui}`Builder` to {gui}`Code`.
   ...
   You can press the {gui}`Run Queries` button to preview the data
   that comes from running this query. You'll see one entry
   for each user, with the value being the % of their limits they
   have used so far.
5. Under {gui}`Alert condition`, specify "0.8" for the text box so it
   reads "When query is above 0.8". Press the {gui}`Preview alert rule condition` button to see what users this alert would fire for.
   Adjust the % at which you'd like alerts by changing this value
   from 0.8 to whatever feels convenient for your community.
6. Under {gui}`Add folder and labels`, select the folder we created
   earlier ("Community Maintained Dashboards") so we can keep
   our alerts organized.
7. Under {gui}`Set evaluation behavior`, select {gui}`New evaluation group`.
   Give it the name "default", and hit {gui}`Create`. You'll only
...
8. Under {gui}`Configure notifications`, select the slack contact point
   you created in the earlier step.
9. Expand the {gui}`Muting, grouping and timings (optional)` section and toggle
   {gui}`Override grouping` to be on. By default, Grafana will generate *one* alert
