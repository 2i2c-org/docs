...
When referring to a user interface, mark elements the reader sees or clicks with ``{gui}`Button text` ``. To show spaced menu paths, separate items with `->` or just `>` (e.g. ``{gui}`One > Two > Three``).
If you need an icon, use ``{icon}`prefix:name` `` (uses the [Iconify](https://icon-sets.iconify.design) plugin).
If you need key-strokes, use ``{kbd}`Buttonname` ``.
